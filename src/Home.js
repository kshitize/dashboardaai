import React, { useState } from "react";
import { NavLink } from "react-router-dom";
import { Helmet } from "react-helmet-async";
import data from "./Airportsfilter.json";

/* ─── inline styles (no extra CSS file needed) ─────────────────────────────── */
const styles = {
  page: {
    fontFamily: "'Inter', sans-serif",
    background: "#F0F4FA",
    minHeight: "100vh",
  },
  hero: {
    background: "linear-gradient(135deg, #0A1628 0%, #0D2247 60%, #1A3A6B 100%)",
    padding: "30px 24px 6px",
    position: "relative",
    overflow: "hidden",
    transition: "padding 0.35s ease",
  },
  heroCollapsed: {
    padding: "28px 24px 32px",
  },
  collapsible: {
    overflow: "hidden",
    transition: "max-height 0.35s ease, opacity 0.25s ease, margin 0.35s ease",
  },
  heroDiagonals: {
    position: "absolute",
    inset: 0,
    backgroundImage:
      "repeating-linear-gradient(120deg, transparent, transparent 60px, rgba(255,255,255,0.03) 60px, rgba(255,255,255,0.03) 61px)",
    pointerEvents: "none",
  },
  heroLabel: {
    fontFamily: "'Rajdhani', sans-serif",
    fontSize: "15px",
    fontWeight: 700,
    letterSpacing: "4px",
    color: "#F5A623",
    textTransform: "uppercase",
    marginBottom: "12px",
    display: "block",
  },
  heroTitle: {
    fontFamily: "'Rajdhani', sans-serif",
    fontSize: "clamp(36px, 6vw, 64px)",
    fontWeight: 700,
    color: "#FFFFFF",
    lineHeight: 1.05,
    marginBottom: "8px",
    letterSpacing: "-0.5px",
  },
  heroSub: {
    color: "rgba(255,255,255,0.5)",
    fontSize: "15px",
    marginBottom: "36px",
    letterSpacing: "0.2px",
  },
  searchWrap: {
    maxWidth: "480px",
    position: "relative",
  },
  searchInput: {
    width: "100%",
    padding: "14px 20px 14px 48px",
    border: "none",
    borderRadius: "10px",
    background: "rgba(255,255,255,0.1)",
    backdropFilter: "blur(8px)",
    color: "#FFFFFF",
    fontSize: "15px",
    outline: "none",
    boxSizing: "border-box",
    transition: "background 0.2s",
  },
  searchIcon: {
    position: "absolute",
    left: "16px",
    top: "50%",
    transform: "translateY(-50%)",
    color: "rgba(255,255,255,0.4)",
    fontSize: "18px",
    pointerEvents: "none",
  },
  statsRow: {
    display: "flex",
    gap: "32px",
    flexWrap: "wrap",
  },
  stat: {
    color: "rgba(255,255,255,0.9)",
  },
  statNum: {
    fontFamily: "'Rajdhani', sans-serif",
    fontSize: "28px",
    fontWeight: 700,
    lineHeight: 1,
    color: "#FFFFFF",
  },
  statLabel: {
    fontSize: "11px",
    letterSpacing: "2px",
    textTransform: "uppercase",
    color: "rgba(255,255,255,0.45)",
    marginTop: "2px",
  },
  body: {
    maxWidth: "1200px",
    margin: "0 auto",
    padding: "40px 20px 80px",
  },
  sectionHeader: {
    display: "flex",
    alignItems: "baseline",
    gap: "12px",
    marginBottom: "28px",
  },
  sectionTitle: {
    fontFamily: "'Rajdhani', sans-serif",
    fontSize: "22px",
    fontWeight: 700,
    color: "#0A1628",
    letterSpacing: "0.5px",
  },
  sectionCount: {
    fontSize: "13px",
    color: "#8A9BB5",
    fontWeight: 500,
  },
  grid: {
    display: "grid",
    gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))",
    gap: "20px",
  },

  /* ── Plate-style card ── */
  plateCard: {
    display: "flex",
    borderRadius: "10px",
    overflow: "hidden",
    textDecoration: "none",
    cursor: "pointer",
    height: "80px", // change the width of the card
    border: "3px solid #E8A417",
    transition: "transform 0.2s ease, box-shadow 0.2s ease",
  },
  plateCodePanel: {
    width: "34%",
    minWidth: "110px",
    background: "linear-gradient(160deg, #2E6FD9 0%, #1B4FA8 55%, #123A82 100%)",
    display: "flex",
    flexDirection: "column",
    alignItems: "center",
    justifyContent: "center",
    gap: "2px",
    borderRight: "3px solid #E8A417",
    position: "relative",
  },
  plateGlossTop: {
    position: "absolute",
    top: 0,
    left: 0,
    right: 0,
    height: "45%",
    background: "linear-gradient(180deg, rgba(255,255,255,0.18), transparent)",
    pointerEvents: "none",
  },
  plateGlossTopGold: {
    position: "absolute",
    top: 0,
    left: 0,
    right: 0,
    height: "45%",
    background: "linear-gradient(180deg, rgba(255,255,255,0.25), transparent)",
    pointerEvents: "none",
  },
  plateIata: {
    fontFamily: "'Rajdhani', sans-serif",
    fontWeight: 700,
    fontSize: "22px",
    color: "#FFFFFF",
    letterSpacing: "1px",
    lineHeight: 1.1,
  },
  plateIcao: {
    fontFamily: "'Rajdhani', sans-serif",
    fontWeight: 700,
    fontSize: "17px",
    color: "#FFFFFF",
    letterSpacing: "1px",
    lineHeight: 1.1,
  },
  plateCityPanel: {
    flex: 1,
    background: "linear-gradient(160deg, #67E8F9 0%, #06B6D4 55%, #0E7490 100%)",
    display: "flex",
    alignItems: "center",
    justifyContent: "center",
    padding: "0 16px",
    position: "relative",
  },
  plateCityText: {
    fontFamily: "'Rajdhani', sans-serif",
    fontWeight: 700,
    fontSize: "clamp(20px, 3vw, 30px)",
    color: "#0A1628",
    letterSpacing: "1px",
    textAlign: "center",
    lineHeight: 1.1,
  },

  emptyState: {
    textAlign: "center",
    padding: "64px 20px",
    color: "#8A9BB5",
  },
  emptyIcon: {
    fontSize: "40px",
    marginBottom: "16px",
  },
  emptyTitle: {
    fontFamily: "'Rajdhani', sans-serif",
    fontSize: "20px",
    fontWeight: 700,
    color: "#0A1628",
    marginBottom: "8px",
  },
};

/* ─── Card component (license-plate style) ──────────────────────────────────── */
const AirportCard = ({ val }) => {
  const [hovered, setHovered] = useState(false);

  const iata = val.iata || "";
  const icao = val.icao || "----";
  // Use only the part before a "/" for display (e.g. "trichy / tiruchirappalli" -> "TRICHY")
  const cityName = val.name.split("/")[0].trim().toUpperCase();

  return (
    <NavLink
      to={`/${val.link}`}
      style={{
        ...styles.plateCard,
        boxShadow: hovered
          ? "0 10px 26px rgba(10,22,40,0.28)"
          : "0 4px 14px rgba(10,22,40,0.16)",
        transform: hovered ? "translateY(-4px)" : "translateY(0)",
      }}
      onMouseEnter={() => setHovered(true)}
      onMouseLeave={() => setHovered(false)}
    >
      {/* ── Blue code panel ── */}
      <div style={styles.plateCodePanel}>
        <div style={styles.plateGlossTop} />
        {iata && <span style={styles.plateIata}>{iata}</span>}
        <span style={styles.plateIcao}>{icao}</span>
      </div>

      {/* ── Gold city panel ── */}
      <div style={styles.plateCityPanel}>
        <div style={styles.plateGlossTopGold} />
        <span style={styles.plateCityText}>{cityName}</span>
      </div>
    </NavLink>
  );
};

/* ─── Home page ─────────────────────────────────────────────────────────────── */
const Home = () => {
  const [searchTerm, setSearchTerm] = useState("");
  const isSearching = searchTerm.trim() !== "";

  const filtered = data.filter((val) =>
    searchTerm === ""
      ? true
      : val.name.toLowerCase().includes(searchTerm.toLowerCase())
  );

  return (
    <>
      <Helmet>
        <title>AAI Airports Dashboard</title>
        <meta name="description" content="Airports Authority of India – Airport Information Dashboard" />
        <link rel="canonical" href="/" />
        {/* Google Fonts */}
        <link rel="preconnect" href="https://fonts.googleapis.com" />
        <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="true" />
        <link
          href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=Inter:wght@400;500;600&display=swap"
          rel="stylesheet"
        />
      </Helmet>

      <div style={styles.page}>

        {/* ── Hero ── */}
        <div style={{ ...styles.hero, ...(isSearching ? styles.heroCollapsed : {}) }}>
          <div
            style={{
              ...styles.heroDiagonals,
              opacity: isSearching ? 0 : 1,
              transition: "opacity 0.35s ease",
            }}
          />
          <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative" }}>

            {/* Label + Title — collapses away while searching */}
            <div
              style={{
                ...styles.collapsible,
                maxHeight: isSearching ? "0px" : "220px",
                opacity: isSearching ? 0 : 1,
              }}
            >
              <span style={styles.heroLabel}>Airports Authority of India</span>
              <h1 style={styles.heroTitle}>
                Airport<br />Dashboard
              </h1>
              {/* <p style={styles.heroSub}>
                Operational data across AAI-managed airports
              </p> */}
            </div>

            {/* search — always visible/interactable, never unmounts */}
            <div style={styles.searchWrap}>
              <span style={styles.searchIcon}>⌕</span>
              <input
                type="text"
                style={styles.searchInput}
                placeholder="Search airports…"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
                onFocus={(e) => (e.target.style.background = "rgba(255,255,255,0.16)")}
                onBlur={(e) => (e.target.style.background = "rgba(255,255,255,0.1)")}
              />
            </div>

            {/* stats — collapses away while searching */}
            <div
              style={{
                ...styles.collapsible,
                maxHeight: isSearching ? "0px" : "80px",
                opacity: isSearching ? 0 : 1,
                marginTop: isSearching ? "0px" : "40px",
              }}
            >
              {/* <div style={styles.statsRow}>
                <div style={styles.stat}>
                  <div style={styles.statNum}>{data.length}</div>
                  <div style={styles.statLabel}>Airports</div>
                </div>
              </div> */}
            </div>
          </div>
        </div>

        {/* ── Grid ── */}
        <div style={styles.body}>
          <div style={styles.sectionHeader}>
            <span style={styles.sectionTitle}>All Airports</span>
            <span style={styles.sectionCount}>
              {filtered.length} of {data.length}
            </span>
          </div>

          {filtered.length > 0 ? (
            <div style={styles.grid}>
              {filtered.map((val) => (
                <AirportCard key={val.id} val={val} />
              ))}
            </div>
          ) : (
            <div style={styles.emptyState}>
              <div style={styles.emptyIcon}>✈</div>
              <div style={styles.emptyTitle}>No airports found</div>
              <p>Try a different search term.</p>
            </div>
          )}
        </div>
      </div>
    </>
  );
};

export default Home;


// import React, { useState } from "react";
// import { NavLink } from "react-router-dom";
// import { Helmet } from "react-helmet-async";
// import data from "./Airportsfilter.json";

// /* ─── inline styles (no extra CSS file needed) ─────────────────────────────── */
// const styles = {
//   page: {
//     fontFamily: "'Inter', sans-serif",
//     background: "#F0F4FA",
//     minHeight: "100vh",
//   },
//   hero: {
//     background: "linear-gradient(135deg, #0A1628 0%, #0D2247 60%, #1A3A6B 100%)",
//     padding: "56px 24px 64px",
//     position: "relative",
//     overflow: "hidden",
//     transition: "padding 0.35s ease",
//   },
//   heroCollapsed: {
//     padding: "28px 24px 32px",
//   },
//   collapsible: {
//     overflow: "hidden",
//     transition: "max-height 0.35s ease, opacity 0.25s ease, margin 0.35s ease",
//   },
//   heroDiagonals: {
//     position: "absolute",
//     inset: 0,
//     backgroundImage:
//       "repeating-linear-gradient(120deg, transparent, transparent 60px, rgba(255,255,255,0.03) 60px, rgba(255,255,255,0.03) 61px)",
//     pointerEvents: "none",
//   },
//   heroLabel: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "15px",
//     fontWeight: 700,
//     letterSpacing: "4px",
//     color: "#F5A623",
//     textTransform: "uppercase",
//     marginBottom: "12px",
//     display: "block",
//   },
//   heroTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "clamp(36px, 6vw, 64px)",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     lineHeight: 1.05,
//     marginBottom: "8px",
//     letterSpacing: "-0.5px",
//   },
//   heroSub: {
//     color: "rgba(255,255,255,0.5)",
//     fontSize: "15px",
//     marginBottom: "36px",
//     letterSpacing: "0.2px",
//   },
//   searchWrap: {
//     maxWidth: "480px",
//     position: "relative",
//   },
//   searchInput: {
//     width: "100%",
//     padding: "14px 20px 14px 48px",
//     border: "none",
//     borderRadius: "10px",
//     background: "rgba(255,255,255,0.1)",
//     backdropFilter: "blur(8px)",
//     color: "#FFFFFF",
//     fontSize: "15px",
//     outline: "none",
//     boxSizing: "border-box",
//     transition: "background 0.2s",
//   },
//   searchIcon: {
//     position: "absolute",
//     left: "16px",
//     top: "50%",
//     transform: "translateY(-50%)",
//     color: "rgba(255,255,255,0.4)",
//     fontSize: "18px",
//     pointerEvents: "none",
//   },
//   statsRow: {
//     display: "flex",
//     gap: "32px",
//     flexWrap: "wrap",
//   },
//   stat: {
//     color: "rgba(255,255,255,0.9)",
//   },
//   statNum: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "28px",
//     fontWeight: 700,
//     lineHeight: 1,
//     color: "#FFFFFF",
//   },
//   statLabel: {
//     fontSize: "11px",
//     letterSpacing: "2px",
//     textTransform: "uppercase",
//     color: "rgba(255,255,255,0.45)",
//     marginTop: "2px",
//   },
//   body: {
//     maxWidth: "1200px",
//     margin: "0 auto",
//     padding: "40px 20px 80px",
//   },
//   sectionHeader: {
//     display: "flex",
//     alignItems: "baseline",
//     gap: "12px",
//     marginBottom: "28px",
//   },
//   sectionTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "22px",
//     fontWeight: 700,
//     color: "#0A1628",
//     letterSpacing: "0.5px",
//   },
//   sectionCount: {
//     fontSize: "13px",
//     color: "#8A9BB5",
//     fontWeight: 500,
//   },
//   grid: {
//     display: "grid",
//     gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))",
//     gap: "20px",
//   },
//   card: {
//     position: "relative",
//     background: "linear-gradient(155deg, #10254A 0%, #0D2247 55%, #0A1628 100%)",
//     borderRadius: "16px",
//     overflow: "hidden",
//     border: "1px solid rgba(255,255,255,0.06)",
//     boxShadow: "0 4px 18px rgba(10,22,40,0.18)",
//     transition: "transform 0.22s ease, box-shadow 0.22s ease, border-color 0.22s ease",
//     display: "flex",
//     flexDirection: "column",
//     cursor: "pointer",
//     textDecoration: "none",
//     color: "inherit",
//     minHeight: "168px",
//   },
//   cardDiagonals: {
//     position: "absolute",
//     inset: 0,
//     backgroundImage:
//       "repeating-linear-gradient(120deg, transparent, transparent 22px, rgba(255,255,255,0.025) 22px, rgba(255,255,255,0.025) 23px)",
//     pointerEvents: "none",
//   },
//   cardWatermark: {
//     position: "absolute",
//     top: "-6px",
//     right: "10px",
//     fontFamily: "'Rajdhani', sans-serif",
//     fontWeight: 700,
//     fontSize: "76px",
//     lineHeight: 1,
//     color: "rgba(255,255,255,0.05)",
//     letterSpacing: "-2px",
//     pointerEvents: "none",
//     userSelect: "none",
//   },
//   cardAccent: {
//     height: "3px",
//     background: "linear-gradient(90deg, #2B7FD4, #F5A623)",
//     flexShrink: 0,
//   },
//   cardBody: {
//     position: "relative",
//     padding: "20px 20px 18px",
//     flex: 1,
//     display: "flex",
//     flexDirection: "column",
//     gap: "14px",
//   },
//   cardStatusRow: {
//     display: "flex",
//     alignItems: "center",
//     gap: "7px",
//   },
//   cardStatusDot: {
//     width: "6px",
//     height: "6px",
//     borderRadius: "50%",
//     background: "#4ADE80",
//     boxShadow: "0 0 0 3px rgba(74,222,128,0.15)",
//     flexShrink: 0,
//   },
//   cardStatusText: {
//     fontSize: "10px",
//     fontWeight: 600,
//     letterSpacing: "1.6px",
//     textTransform: "uppercase",
//     color: "rgba(255,255,255,0.4)",
//   },
//   cardTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "21px",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     letterSpacing: "0.3px",
//     lineHeight: 1.15,
//     margin: 0,
//   },
//   cardMeta: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     color: "#8A9BB5",
//   },
//   metaDot: {
//     width: "5px",
//     height: "5px",
//     borderRadius: "50%",
//     background: "#F5A623",
//     flexShrink: 0,
//   },
//   cardFooter: {
//     marginTop: "auto",
//     paddingTop: "14px",
//     borderTop: "1px solid rgba(255,255,255,0.08)",
//     display: "flex",
//     justifyContent: "space-between",
//     alignItems: "center",
//   },
//   updatedBadge: {
//     fontSize: "11px",
//     color: "rgba(255,255,255,0.35)",
//     letterSpacing: "0.2px",
//   },
//   arrowBtn: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     fontWeight: 600,
//     color: "#F5A623",
//     letterSpacing: "0.3px",
//     textTransform: "uppercase",
//     textDecoration: "none",
//     transition: "gap 0.15s",
//   },
//   emptyState: {
//     textAlign: "center",
//     padding: "64px 20px",
//     color: "#8A9BB5",
//   },
//   emptyIcon: {
//     fontSize: "40px",
//     marginBottom: "16px",
//   },
//   emptyTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "20px",
//     fontWeight: 700,
//     color: "#0A1628",
//     marginBottom: "8px",
//   },
// };

// /* ─── Card component ────────────────────────────────────────────────────────── */
// const AirportCard = ({ val }) => {
//   const [hovered, setHovered] = useState(false);

//   // faux IATA-style watermark code, e.g. "Indira Gandhi" -> "IND"
//   const code = val.name.replace(/[^a-zA-Z]/g, "").slice(0, 3).toUpperCase();

//   return (
//     <NavLink
//       to={`/${val.link}`}
//       style={{
//         ...styles.card,
//         transform: hovered ? "translateY(-5px)" : "translateY(0)",
//         boxShadow: hovered
//           ? "0 14px 34px rgba(10,22,40,0.28), 0 0 0 1px rgba(245,166,35,0.25)"
//           : "0 4px 18px rgba(10,22,40,0.18)",
//         borderColor: hovered ? "rgba(245,166,35,0.3)" : "rgba(255,255,255,0.06)",
//       }}
//       onMouseEnter={() => setHovered(true)}
//       onMouseLeave={() => setHovered(false)}
//     >
//       {/* subtle texture + watermark, echoing the hero */}
//       <div style={styles.cardDiagonals} />
//       {/* <div style={styles.cardWatermark}>{code}</div> */}

//       {/* accent bar */}
//       <div style={styles.cardAccent} />

//       {/* body */}
//       <div style={styles.cardBody}>
//         <div style={styles.cardStatusRow}>
//           {/* <span style={styles.cardStatusDot} />
//           <span style={styles.cardStatusText}>Operational</span> */}
//         </div>

//         <h3 style={styles.cardTitle}>{val.name.toUpperCase()}</h3>

//         <div style={styles.cardFooter}>
//           <span style={styles.updatedBadge}>
//             {/* Updated {val.dateofupdation} */}
//           </span>
//           <span
//             style={{
//               ...styles.arrowBtn,
//               gap: hovered ? "9px" : "6px",
//             }}
//           >
//             View details
//             <span style={{ fontSize: "16px", lineHeight: 1 }}>→</span>
//           </span>
//         </div>
//       </div>
//     </NavLink>
//   );
// };

// /* ─── Home page ─────────────────────────────────────────────────────────────── */
// const Home = () => {
//   const [searchTerm, setSearchTerm] = useState("");
//   const isSearching = searchTerm.trim() !== "";

//   const filtered = data.filter((val) =>
//     searchTerm === ""
//       ? true
//       : val.name.toLowerCase().includes(searchTerm.toLowerCase())
//   );

//   return (
//     <>
//       <Helmet>
//         <title>AAI Airports Dashboard</title>
//         <meta name="description" content="Airports Authority of India – Airport Information Dashboard" />
//         <link rel="canonical" href="/" />
//         {/* Google Fonts */}
//         <link rel="preconnect" href="https://fonts.googleapis.com" />
//         <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="true" />
//         <link
//           href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=Inter:wght@400;500;600&display=swap"
//           rel="stylesheet"
//         />
//       </Helmet>

//       <div style={styles.page}>

//         {/* ── Hero ── */}
//         <div style={{ ...styles.hero, ...(isSearching ? styles.heroCollapsed : {}) }}>
//           <div
//             style={{
//               ...styles.heroDiagonals,
//               opacity: isSearching ? 0 : 1,
//               transition: "opacity 0.35s ease",
//             }}
//           />
//           <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative" }}>

//             {/* Label + Title — collapses away while searching */}
//             <div
//               style={{
//                 ...styles.collapsible,
//                 maxHeight: isSearching ? "0px" : "220px",
//                 opacity: isSearching ? 0 : 1,
//               }}
//             >
//               <span style={styles.heroLabel}>Airports Authority of India</span>
//               <h1 style={styles.heroTitle}>
//                 Airport<br />Dashboard
//               </h1>
//               <p style={styles.heroSub}>
//                 Operational data across AAI-managed airports
//               </p>
//             </div>

//             {/* search — always visible/interactable, never unmounts */}
//             <div style={styles.searchWrap}>
//               <span style={styles.searchIcon}>⌕</span>
//               <input
//                 type="text"
//                 style={styles.searchInput}
//                 placeholder="Search airports…"
//                 value={searchTerm}
//                 onChange={(e) => setSearchTerm(e.target.value)}
//                 onFocus={(e) => (e.target.style.background = "rgba(255,255,255,0.16)")}
//                 onBlur={(e) => (e.target.style.background = "rgba(255,255,255,0.1)")}
//               />
//             </div>

//             {/* stats — collapses away while searching */}
//             <div
//               style={{
//                 ...styles.collapsible,
//                 maxHeight: isSearching ? "0px" : "80px",
//                 opacity: isSearching ? 0 : 1,
//                 marginTop: isSearching ? "0px" : "40px",
//               }}
//             >
//               <div style={styles.statsRow}>
//                 <div style={styles.stat}>
//                   <div style={styles.statNum}>{data.length}</div>
//                   <div style={styles.statLabel}>Airports</div>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </div>

//         {/* ── Grid ── */}
//         <div style={styles.body}>
//           <div style={styles.sectionHeader}>
//             <span style={styles.sectionTitle}>All Airports</span>
//             <span style={styles.sectionCount}>
//               {filtered.length} of {data.length}
//             </span>
//           </div>

//           {filtered.length > 0 ? (
//             <div style={styles.grid}>
//               {filtered.map((val) => (
//                 <AirportCard key={val.id} val={val} />
//               ))}
//             </div>
//           ) : (
//             <div style={styles.emptyState}>
//               <div style={styles.emptyIcon}>✈</div>
//               <div style={styles.emptyTitle}>No airports found</div>
//               <p>Try a different search term.</p>
//             </div>
//           )}
//         </div>
//       </div>
//     </>
//   );
// };

// export default Home;

// import React, { useState } from "react";
// import { NavLink } from "react-router-dom";
// import { Helmet } from "react-helmet-async";
// import data from "./Airportsfilter.json";

// /* ─── inline styles (no extra CSS file needed) ─────────────────────────────── */
// const styles = {
//   page: {
//     fontFamily: "'Inter', sans-serif",
//     background: "#F0F4FA",
//     minHeight: "100vh",
//   },
//   hero: {
//     background: "linear-gradient(135deg, #0A1628 0%, #0D2247 60%, #1A3A6B 100%)",
//     padding: "56px 24px 64px",
//     position: "relative",
//     overflow: "hidden",
//     transition: "padding 0.35s ease",
//   },
//   heroCollapsed: {
//     padding: "28px 24px 32px",
//   },
//   collapsible: {
//     overflow: "hidden",
//     transition: "max-height 0.35s ease, opacity 0.25s ease, margin 0.35s ease",
//   },
//   heroDiagonals: {
//     position: "absolute",
//     inset: 0,
//     backgroundImage:
//       "repeating-linear-gradient(120deg, transparent, transparent 60px, rgba(255,255,255,0.03) 60px, rgba(255,255,255,0.03) 61px)",
//     pointerEvents: "none",
//   },
//   heroLabel: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "15px",
//     fontWeight: 700,
//     letterSpacing: "4px",
//     color: "#F5A623",
//     textTransform: "uppercase",
//     marginBottom: "12px",
//     display: "block",
//   },
//   heroTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "clamp(36px, 6vw, 64px)",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     lineHeight: 1.05,
//     marginBottom: "8px",
//     letterSpacing: "-0.5px",
//   },
//   heroSub: {
//     color: "rgba(255,255,255,0.5)",
//     fontSize: "15px",
//     marginBottom: "36px",
//     letterSpacing: "0.2px",
//   },
//   searchWrap: {
//     maxWidth: "480px",
//     position: "relative",
//   },
//   searchInput: {
//     width: "100%",
//     padding: "14px 20px 14px 48px",
//     border: "none",
//     borderRadius: "10px",
//     background: "rgba(255,255,255,0.1)",
//     backdropFilter: "blur(8px)",
//     color: "#FFFFFF",
//     fontSize: "15px",
//     outline: "none",
//     boxSizing: "border-box",
//     transition: "background 0.2s",
//   },
//   searchIcon: {
//     position: "absolute",
//     left: "16px",
//     top: "50%",
//     transform: "translateY(-50%)",
//     color: "rgba(255,255,255,0.4)",
//     fontSize: "18px",
//     pointerEvents: "none",
//   },
//   statsRow: {
//     display: "flex",
//     gap: "32px",
//     flexWrap: "wrap",
//   },
//   stat: {
//     color: "rgba(255,255,255,0.9)",
//   },
//   statNum: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "28px",
//     fontWeight: 700,
//     lineHeight: 1,
//     color: "#FFFFFF",
//   },
//   statLabel: {
//     fontSize: "11px",
//     letterSpacing: "2px",
//     textTransform: "uppercase",
//     color: "rgba(255,255,255,0.45)",
//     marginTop: "2px",
//   },
//   body: {
//     maxWidth: "1200px",
//     margin: "0 auto",
//     padding: "40px 20px 80px",
//   },
//   sectionHeader: {
//     display: "flex",
//     alignItems: "baseline",
//     gap: "12px",
//     marginBottom: "28px",
//   },
//   sectionTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "22px",
//     fontWeight: 700,
//     color: "#0A1628",
//     letterSpacing: "0.5px",
//   },
//   sectionCount: {
//     fontSize: "13px",
//     color: "#8A9BB5",
//     fontWeight: 500,
//   },
//   grid: {
//     display: "grid",
//     gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))",
//     gap: "20px",
//   },
//   card: {
//     background: "linear-gradient(135deg, #f0f0f0 0%, #f0f0f0 60%, #f0f0f0 100%)",
//     borderRadius: "14px",
//     overflow: "hidden",
//     boxShadow: "0 2px 12px rgba(10,22,40,0.07)",
//     transition: "transform 0.18s ease, box-shadow 0.18s ease",
//     display: "flex",
//     flexDirection: "column",
//     cursor: "pointer",
//     textDecoration: "none",
//     color: "inherit",
//   },
//   cardImgWrap: {
//     position: "relative",
//     height: "160px",
//     overflow: "hidden",
//     background: "#0D2247",
//   },
//   cardImg: {
//     width: "100%",
//     height: "100%",
//     objectFit: "cover",
//     display: "block",
//     transition: "transform 0.3s ease",
//   },
//   cardImgOverlay: {
//     position: "absolute",
//     inset: 0,
//     background: "linear-gradient(to top, rgba(10,22,40,0.65) 0%, transparent 60%)",
//   },
//   cardImgLabel: {
//     position: "absolute",
//     bottom: "12px",
//     left: "14px",
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "20px",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     letterSpacing: "1px",
//     lineHeight: 1,
//   },
//   cardAccent: {
//     height: "3px",
//     background: "linear-gradient(90deg, #2B7FD4, #F5A623)",
//   },
//   cardBody: {
//     padding: "16px 18px 18px",
//     flex: 1,
//     display: "flex",
//     flexDirection: "column",
//     gap: "10px",
//   },
//   cardTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "17px",
//     fontWeight: 700,
//     color: "#0A1628",
//     letterSpacing: "0.3px",
//     margin: 0,
//   },
//   cardMeta: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     color: "#8A9BB5",
//   },
//   metaDot: {
//     width: "5px",
//     height: "5px",
//     borderRadius: "50%",
//     background: "#F5A623",
//     flexShrink: 0,
//   },
//   cardFooter: {
//     marginTop: "auto",
//     paddingTop: "12px",
//     borderTop: "1px solid #F0F4FA",
//     display: "flex",
//     justifyContent: "space-between",
//     alignItems: "center",
//   },
//   updatedBadge: {
//     fontSize: "11px",
//     color: "#8A9BB5",
//     letterSpacing: "0.2px",
//   },
//   arrowBtn: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     fontWeight: 600,
//     color: "#2B7FD4",
//     letterSpacing: "0.3px",
//     textTransform: "uppercase",
//     textDecoration: "none",
//     transition: "gap 0.15s",
//   },
//   emptyState: {
//     textAlign: "center",
//     padding: "64px 20px",
//     color: "#8A9BB5",
//   },
//   emptyIcon: {
//     fontSize: "40px",
//     marginBottom: "16px",
//   },
//   emptyTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "20px",
//     fontWeight: 700,
//     color: "#0A1628",
//     marginBottom: "8px",
//   },
// };

// /* ─── Card component ────────────────────────────────────────────────────────── */
// const AirportCard = ({ val }) => {
//   const [hovered, setHovered] = useState(false);

//   return (
//     <div
//       style={{
//         ...styles.card,
//         transform: hovered ? "translateY(-4px)" : "translateY(0)",
//         boxShadow: hovered
//           ? "0 12px 32px rgba(10,22,40,0.13)"
//           : "0 2px 12px rgba(10,22,40,0.07)",
//       }}
//       onMouseEnter={() => setHovered(true)}
//       onMouseLeave={() => setHovered(false)}
//     >
//       {/* image */}
//       {/* <div style={styles.cardImgWrap}>
//         <img
//           src={require(`${val.image}`)}
//           style={{
//             ...styles.cardImg,
//             transform: hovered ? "scale(1.04)" : "scale(1)",
//           }}
//           alt={val.name}
//         />
//         <div style={styles.cardImgOverlay} />
//         <div style={styles.cardImgLabel}>
//           {val.name.split(" ")[0].toUpperCase()}
//         </div>
//       </div> */}

//       {/* accent bar */}
//       <div style={styles.cardAccent} />

//       {/* body */}
//       <div style={styles.cardBody}>
//         <h3 style={styles.cardTitle}>{val.name.toUpperCase()}</h3>

//         <div style={styles.cardFooter}>
//           <span style={styles.updatedBadge}>
//             {/* Updated {val.dateofupdation} */}
//           </span>
//           <NavLink to={`/${val.link}`} style={styles.arrowBtn}>
//             View details
//             <span style={{ fontSize: "16px", lineHeight: 1 }}>→</span>
//           </NavLink>
//         </div>
//       </div>
//     </div>
//   );
// };

// /* ─── Home page ─────────────────────────────────────────────────────────────── */
// const Home = () => {
//   const [searchTerm, setSearchTerm] = useState("");
//   const isSearching = searchTerm.trim() !== "";

//   const filtered = data.filter((val) =>
//     searchTerm === ""
//       ? true
//       : val.name.toLowerCase().includes(searchTerm.toLowerCase())
//   );

//   return (
//     <>
//       <Helmet>
//         <title>AAI Airports Dashboard</title>
//         <meta name="description" content="Airports Authority of India – Airport Information Dashboard" />
//         <link rel="canonical" href="/" />
//         {/* Google Fonts */}
//         <link rel="preconnect" href="https://fonts.googleapis.com" />
//         <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="true" />
//         <link
//           href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=Inter:wght@400;500;600&display=swap"
//           rel="stylesheet"
//         />
//       </Helmet>

//       <div style={styles.page}>

//         {/* ── Hero ── */}
//         <div style={{ ...styles.hero, ...(isSearching ? styles.heroCollapsed : {}) }}>
//           <div
//             style={{
//               ...styles.heroDiagonals,
//               opacity: isSearching ? 0 : 1,
//               transition: "opacity 0.35s ease",
//             }}
//           />
//           <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative" }}>

//             {/* Label + Title — collapses away while searching */}
//             <div
//               style={{
//                 ...styles.collapsible,
//                 maxHeight: isSearching ? "0px" : "220px",
//                 opacity: isSearching ? 0 : 1,
//               }}
//             >
//               <span style={styles.heroLabel}>Airports Authority of India</span>
//               <h1 style={styles.heroTitle}>
//                 Airport<br />Dashboard
//               </h1>
//               <p style={styles.heroSub}>
//                 Operational data across AAI-managed airports
//               </p>
//             </div>

//             {/* search — always visible/interactable, never unmounts */}
//             <div style={styles.searchWrap}>
//               <span style={styles.searchIcon}>⌕</span>
//               <input
//                 type="text"
//                 style={styles.searchInput}
//                 placeholder="Search airports…"
//                 value={searchTerm}
//                 onChange={(e) => setSearchTerm(e.target.value)}
//                 onFocus={(e) => (e.target.style.background = "rgba(255,255,255,0.16)")}
//                 onBlur={(e) => (e.target.style.background = "rgba(255,255,255,0.1)")}
//               />
//             </div>

//             {/* stats — collapses away while searching */}
//             <div
//               style={{
//                 ...styles.collapsible,
//                 maxHeight: isSearching ? "0px" : "80px",
//                 opacity: isSearching ? 0 : 1,
//                 marginTop: isSearching ? "0px" : "40px",
//               }}
//             >
//               <div style={styles.statsRow}>
//                 <div style={styles.stat}>
//                   <div style={styles.statNum}>{data.length}</div>
//                   <div style={styles.statLabel}>Airports</div>
//                 </div>
//               </div>
//             </div>
//           </div>
//         </div>

//         {/* ── Grid ── */}
//         <div style={styles.body}>
//           <div style={styles.sectionHeader}>
//             <span style={styles.sectionTitle}>All Airports</span>
//             <span style={styles.sectionCount}>
//               {filtered.length} of {data.length}
//             </span>
//           </div>

//           {filtered.length > 0 ? (
//             <div style={styles.grid}>
//               {filtered.map((val) => (
//                 <AirportCard key={val.id} val={val} />
//               ))}
//             </div>
//           ) : (
//             <div style={styles.emptyState}>
//               <div style={styles.emptyIcon}>✈</div>
//               <div style={styles.emptyTitle}>No airports found</div>
//               <p>Try a different search term.</p>
//             </div>
//           )}
//         </div>
//       </div>
//     </>
//   );
// };

// export default Home;



// import React, { useState } from "react";
// import { NavLink } from "react-router-dom";
// import { Helmet } from "react-helmet-async";
// import data from "./Airportsfilter.json";

// /* ─── inline styles (no extra CSS file needed) ─────────────────────────────── */
// const styles = {
//   page: {
//     fontFamily: "'Inter', sans-serif",
//     background: "#F0F4FA",
//     minHeight: "100vh",
//   },
//   hero: {
//     background: "linear-gradient(135deg, #0A1628 0%, #0D2247 60%, #1A3A6B 100%)",
//     padding: "56px 24px 64px",
//     position: "relative",
//     overflow: "hidden",
//   },
//   heroDiagonals: {
//     position: "absolute",
//     inset: 0,
//     backgroundImage:
//       "repeating-linear-gradient(120deg, transparent, transparent 60px, rgba(255,255,255,0.03) 60px, rgba(255,255,255,0.03) 61px)",
//     pointerEvents: "none",
//   },
//   heroLabel: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "15px",
//     fontWeight: 700,
//     letterSpacing: "4px",
//     color: "#F5A623",
//     textTransform: "uppercase",
//     marginBottom: "12px",
//     display: "block",
//   },
//   heroTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "clamp(36px, 6vw, 64px)",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     lineHeight: 1.05,
//     marginBottom: "8px",
//     letterSpacing: "-0.5px",
//   },
//   heroSub: {
//     color: "rgba(255,255,255,0.5)",
//     fontSize: "15px",
//     marginBottom: "36px",
//     letterSpacing: "0.2px",
//   },
//   searchWrap: {
//     maxWidth: "480px",
//     position: "relative",
//   },
//   searchInput: {
//     width: "100%",
//     padding: "14px 20px 14px 48px",
//     border: "none",
//     borderRadius: "10px",
//     background: "rgba(255,255,255,0.1)",
//     backdropFilter: "blur(8px)",
//     color: "#FFFFFF",
//     fontSize: "15px",
//     outline: "none",
//     boxSizing: "border-box",
//     transition: "background 0.2s",
//   },
//   searchIcon: {
//     position: "absolute",
//     left: "16px",
//     top: "50%",
//     transform: "translateY(-50%)",
//     color: "rgba(255,255,255,0.4)",
//     fontSize: "18px",
//     pointerEvents: "none",
//   },
//   statsRow: {
//     display: "flex",
//     gap: "32px",
//     marginTop: "40px",
//     flexWrap: "wrap",
//   },
//   stat: {
//     color: "rgba(255,255,255,0.9)",
//   },
//   statNum: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "28px",
//     fontWeight: 700,
//     lineHeight: 1,
//     color: "#FFFFFF",
//   },
//   statLabel: {
//     fontSize: "11px",
//     letterSpacing: "2px",
//     textTransform: "uppercase",
//     color: "rgba(255,255,255,0.45)",
//     marginTop: "2px",
//   },
//   body: {
//     maxWidth: "1200px",
//     margin: "0 auto",
//     padding: "40px 20px 80px",
//   },
//   sectionHeader: {
//     display: "flex",
//     alignItems: "baseline",
//     gap: "12px",
//     marginBottom: "28px",
//   },
//   sectionTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "22px",
//     fontWeight: 700,
//     color: "#0A1628",
//     letterSpacing: "0.5px",
//   },
//   sectionCount: {
//     fontSize: "13px",
//     color: "#8A9BB5",
//     fontWeight: 500,
//   },
//   grid: {
//     display: "grid",
//     gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))",
//     gap: "20px",
//   },
//   card: {
//     // background: "#f0f0f0",
//     background: "linear-gradient(135deg, #f0f0f0 0%, #f0f0f0 60%, #f0f0f0 100%)",
//     borderRadius: "14px",
//     overflow: "hidden",
//     boxShadow: "0 2px 12px rgba(10,22,40,0.07)",
//     transition: "transform 0.18s ease, box-shadow 0.18s ease",
//     display: "flex",
//     flexDirection: "column",
//     cursor: "pointer",
//     textDecoration: "none",
//     color: "inherit",
//   },
//   cardImgWrap: {
//     position: "relative",
//     height: "160px",
//     overflow: "hidden",
//     background: "#0D2247",
//   },
//   cardImg: {
//     width: "100%",
//     height: "100%",
//     objectFit: "cover",
//     display: "block",
//     transition: "transform 0.3s ease",
//   },
//   cardImgOverlay: {
//     position: "absolute",
//     inset: 0,
//     background: "linear-gradient(to top, rgba(10,22,40,0.65) 0%, transparent 60%)",
//   },
//   cardImgLabel: {
//     position: "absolute",
//     bottom: "12px",
//     left: "14px",
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "20px",
//     fontWeight: 700,
//     color: "#FFFFFF",
//     letterSpacing: "1px",
//     lineHeight: 1,
//   },
//   cardAccent: {
//     height: "3px",
//     background: "linear-gradient(90deg, #2B7FD4, #F5A623)",
//   },
//   cardBody: {
//     padding: "16px 18px 18px",
//     flex: 1,
//     display: "flex",
//     flexDirection: "column",
//     gap: "10px",
//   },
//   cardTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "17px",
//     fontWeight: 700,
//     color: "#0A1628",
//     letterSpacing: "0.3px",
//     margin: 0,
//   },
//   cardMeta: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     color: "#8A9BB5",
//   },
//   metaDot: {
//     width: "5px",
//     height: "5px",
//     borderRadius: "50%",
//     background: "#F5A623",
//     flexShrink: 0,
//   },
//   cardFooter: {
//     marginTop: "auto",
//     paddingTop: "12px",
//     borderTop: "1px solid #F0F4FA",
//     display: "flex",
//     justifyContent: "space-between",
//     alignItems: "center",
//   },
//   updatedBadge: {
//     fontSize: "11px",
//     color: "#8A9BB5",
//     letterSpacing: "0.2px",
//   },
//   arrowBtn: {
//     display: "flex",
//     alignItems: "center",
//     gap: "6px",
//     fontSize: "12px",
//     fontWeight: 600,
//     color: "#2B7FD4",
//     letterSpacing: "0.3px",
//     textTransform: "uppercase",
//     textDecoration: "none",
//     transition: "gap 0.15s",
//   },
//   emptyState: {
//     textAlign: "center",
//     padding: "64px 20px",
//     color: "#8A9BB5",
//   },
//   emptyIcon: {
//     fontSize: "40px",
//     marginBottom: "16px",
//   },
//   emptyTitle: {
//     fontFamily: "'Rajdhani', sans-serif",
//     fontSize: "20px",
//     fontWeight: 700,
//     color: "#0A1628",
//     marginBottom: "8px",
//   },
// };

// /* ─── Card component ────────────────────────────────────────────────────────── */
// const AirportCard = ({ val }) => {
//   const [hovered, setHovered] = useState(false);

//   return (
//     <div
//       style={{
//         ...styles.card,
//         transform: hovered ? "translateY(-4px)" : "translateY(0)",
//         boxShadow: hovered
//           ? "0 12px 32px rgba(10,22,40,0.13)"
//           : "0 2px 12px rgba(10,22,40,0.07)",
//       }}
//       onMouseEnter={() => setHovered(true)}
//       onMouseLeave={() => setHovered(false)}
//     >
//       {/* image */}
//       {/* <div style={styles.cardImgWrap}>
//         <img
//           src={require(`${val.image}`)}
//           style={{
//             ...styles.cardImg,
//             transform: hovered ? "scale(1.04)" : "scale(1)",
//           }}
//           alt={val.name}
//         />
//         <div style={styles.cardImgOverlay} />
//         <div style={styles.cardImgLabel}>
//           {val.name.split(" ")[0].toUpperCase()}
//         </div>
//       </div> */}

//       {/* accent bar */}
//       <div style={styles.cardAccent} />

//       {/* body */}
//       <div style={styles.cardBody}>
//         <h3 style={styles.cardTitle}>{val.name.toUpperCase()}</h3>

//         <div style={styles.cardFooter}>
//           <span style={styles.updatedBadge}>
//             {/* Updated {val.dateofupdation} */}
//           </span>
//           <NavLink to={`/${val.link}`} style={styles.arrowBtn}>
//             View details
//             <span style={{ fontSize: "16px", lineHeight: 1 }}>→</span>
//           </NavLink>
//         </div>
//       </div>
//     </div>
//   );
// };

// /* ─── Home page ─────────────────────────────────────────────────────────────── */
// const Home = () => {
//   const [searchTerm, setSearchTerm] = useState("");

//   const filtered = data.filter((val) =>
//     searchTerm === ""
//       ? true
//       : val.name.toLowerCase().includes(searchTerm.toLowerCase())
//   );

//   return (
//     <>
//       <Helmet>
//         <title>AAI Airports Dashboard</title>
//         <meta name="description" content="Airports Authority of India – Airport Information Dashboard" />
//         <link rel="canonical" href="/" />
//         {/* Google Fonts */}
//         <link rel="preconnect" href="https://fonts.googleapis.com" />
//         <link rel="preconnect" href="https://fonts.gstatic.com" crossOrigin="true" />
//         <link
//           href="https://fonts.googleapis.com/css2?family=Rajdhani:wght@600;700&family=Inter:wght@400;500;600&display=swap"
//           rel="stylesheet"
//         />
//       </Helmet>

//       <div style={styles.page}>

//         {/* ── Hero ── */}
//         <div style={styles.hero}>
//           <div style={styles.heroDiagonals} />
//           <div style={{ maxWidth: "1200px", margin: "0 auto", position: "relative" }}>

//             <span style={styles.heroLabel}>Airports Authority of India</span>
//             <h1 style={styles.heroTitle}>
//               Airport<br />Dashboard
//             </h1>
//             <p style={styles.heroSub}>
//               Operational data across AAI-managed airports
//             </p>

//             {/* search */}
//             <div style={styles.searchWrap}>
//               <span style={styles.searchIcon}>⌕</span>
//               <input
//                 type="text"
//                 style={styles.searchInput}
//                 placeholder="Search airports…"
//                 value={searchTerm}
//                 onChange={(e) => setSearchTerm(e.target.value)}
//                 onFocus={(e) => (e.target.style.background = "rgba(255,255,255,0.16)")}
//                 onBlur={(e) => (e.target.style.background = "rgba(255,255,255,0.1)")}
//               />
//             </div>

//             {/* stats */}
//             <div style={styles.statsRow}>
//               <div style={styles.stat}>
//                 <div style={styles.statNum}>{data.length}</div>
//                 <div style={styles.statLabel}>Airports</div>
//               </div>
//               </div>
//           </div>
//         </div>

//         {/* ── Grid ── */}
//         <div style={styles.body}>
//           <div style={styles.sectionHeader}>
//             <span style={styles.sectionTitle}>All Airports</span>
//             <span style={styles.sectionCount}>
//               {filtered.length} of {data.length}
//             </span>
//           </div>

//           {filtered.length > 0 ? (
//             <div style={styles.grid}>
//               {filtered.map((val) => (
//                 <AirportCard key={val.id} val={val} />
//               ))}
//             </div>
//           ) : (
//             <div style={styles.emptyState}>
//               <div style={styles.emptyIcon}>✈</div>
//               <div style={styles.emptyTitle}>No airports found</div>
//               <p>Try a different search term.</p>
//             </div>
//           )}
//         </div>
//       </div>
//     </>
//   );
// };

// export default Home;