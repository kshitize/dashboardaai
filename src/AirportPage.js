...
const InfoRow = ({ label, value, last = false }) => {
  ...
  return (
    <div className={`row g-2 py-2 ${last ? "" : "border-bottom"}`}>
      <div className="col-12 col-md-4 fw-semibold text-start">{label}</div>
...
// -- Field groupings used both for on-screen accordion and PDF export ------
const GENERAL_FIELDS = [
  ["APD Name", "APD Name"],
  ["Date of Updation", "Updated On"],
  ["APD Phone Number", "Airport Director Phone"],
  ["Terminal Manager Phone", "Terminal Manager Phone"],
  ["APD Mail", "Email"],
  ["Type of Airport", "Type of Airport"],
  ["Watch Hour", "Watch Hour"],
];

const TERMINAL_FIELDS = [
  ["No. of Check-in Counters", "No. of Check-in Counters"],
  ["No. of CUSS Kiosks available", "No. of CUSS Kiosks Available"],
  ["ILBS Facility", "ILBS Facility"],
  ["No. of Aero Bridges", "No. of Aero Bridges"],
  ["Child Care Rooms", "Child Care Rooms"],
  ["May I Help You Counters", "May I Help You Counters"],
  ["Accessible Facilities for PRM", "Accessible Facilities for PRM"],
  ["Reserved Lounges", "Reserved Lounges"],
  ["Digi Yatra Facility", "Digi Yatra Facility"],
  ["Immigration Counters", "Immigration Counters"],
  ["Immigration Manpower", "Immigration Manpower"],
  ["Custom Counters", "Custom Counters"],
  ["Custom Manpower", "Custom Manpower"],
  ["APHO Facility", "APHO Facility"],
  ["Animal Quarantine Facility", "Animal Quarantine Facility"],
  ["Plant Quarantine Facility", "Plant Quarantine Facility"],
  ["Sanitary Napkin Vending Machines", "Sanitary Napkin Vending Machines"],
  ["Solid Waste Management Facility", "Solid Waste Management Facility"],
  ["MI Room", "MI Room"],
  ["BA Test Facility", "BA Test Facility"],
  ["Number of Shifts in Terminal", "Number of Shifts in Terminal"],
];

const AIRSIDE_FIELDS = [
  ["Runway Orientation", "Runway Orientation"],
  ["RWY Dimension", "RWY Dimension"],
  ["RWY Strip Dimension", "RWY Strip Dimension"],
  ["RESA", "RESA"],
  ["Number of TWY", "Number of TWY"],
  ["Aircraft Stands with PBB", "Aircraft Stands with PBB"],
  ["Total Aircraft Stands", "Total Aircraft Stands"],
  ["Approach Light Category", "Approach Light Category"],
  ["Instrument Landing System", "Instrument Landing System"],
  ["LVP/LVTO", "LVP/LVTO"],
  ["Homing Aid", "Homing Aid"],
  ["IFR/VFR", "IFR/VFR"],
  ["Night Landing", "Night Landing"],
  ["Critical Aircraft", "Critical Aircraft"],
  ["Declared Distances", "Declared Distances"],
  ["Instrument Approach", "Instrument Approach"],
  ["Fire Category", "Fire Category"],
];

// -- PDF export helper --------------------------------------------------
const exportAirportPDF = (d, airportName) => {
  const doc = new jsPDF({ unit: "pt", format: "a4" });
  const airportTitle = d["Airport"] || airportName;
  const margin = 40;
  let cursorY = 50;

  doc.setFontSize(18);
  doc.setFont(undefined, "bold");
  doc.text(airportTitle + " Airport - Brief", margin, cursorY);
  ...
  const addSection = (title, fields) => {
    autoTable(doc, {
      startY: cursorY + 15,
      head: [[title, ""]],
      body: buildRows(fields),
      theme: "grid",
      margin: { left: margin, right: margin },
      headStyles: { fillColor: [13, 27, 62], textColor: 255, fontSize: 11 },
  ...
  addSection("General Information", GENERAL_FIELDS);
  addSection("Terminal Information", TERMINAL_FIELDS);
  addSection("Airside Information", AIRSIDE_FIELDS);

  doc.save(airportTitle + " Brief.pdf");
};
          ...
          <div className="card-body p-4">
            <div className="d-flex justify-content-between align-items-start flex-wrap gap-2">
              ...
              <button
                ...
                onClick={() => exportAirportPDF(d, airportName)}
              >
                <FiDownload />
                Export Brief (PDF)
              </button>
      ...
      maxWidth: "100%",
        ...
        <div className="accordion shadow-sm" id={`accordionAirport-${airportName}`}>
          <div className="accordion-item border-0 rounded-4 overflow-hidden mb-3 shadow-sm">
            ...
            >
              <div className="accordion-body">
                {TERMINAL_FIELDS.map(([key, label], i) => (
                  <InfoRow
                    key={key}
                    label={label}
                    value={d[key]}
                    last={i === TERMINAL_FIELDS.length - 1}
                  />
                ))}
              </div>
            </div>
          ...
          <div className="accordion-item border-0 rounded-4 overflow-hidden shadow-sm">
            ...
            >
              <div className="accordion-body">
                {AIRSIDE_FIELDS.map(([key, label], i) => (
                  <InfoRow
                    key={key}
                    label={label}
                    value={d[key]}
                    last={i === AIRSIDE_FIELDS.length - 1}
                  />
                ))}
              </div>
            </div>
